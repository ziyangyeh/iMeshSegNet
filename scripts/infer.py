...
def infer(cfg_path: str, ckpt_path: str, mesh_file: str, refine: bool, device='cuda') -> vedo.Mesh:
    ...
    with torch.no_grad():
        if cfg.model.name == "iMeshSegNet":
            tensor_prob_output = model(input_data["input"], input_data["KG_12"],input_data["KG_6"])
        elif cfg.model.name == "MeshSegNet":
            tensor_prob_output = model(input_data["input"], input_data["A_S"],input_data["A_L"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-gco", "--pygco", type=ast.literal_eval, metavar="", help="pygco", required=True)
