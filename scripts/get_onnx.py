...
def get_onnx(cfg_path: str, ckpt_path: str, mesh_file: str, filepath: str):
    ...
    with torch.no_grad():
        if cfg.model.name == "iMeshSegNet":
            ...
            input_sample_shape = (tuple(input_shape), tuple(KG_12_shape), tuple(KG_6_shape))
        elif cfg.model.name == "MeshSegNet":
            input_shape = input_data["input"].shape
            A_S_shape = input_data["A_S"].shape
            A_L_shape = input_data["A_L"].shape
            input_sample_shape = (tuple(input_shape), tuple(A_S_shape), tuple(A_L_shape))

    x = tuple([torch.randn(i, requires_grad=True, device=device) for i in input_sample_shape])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ckpt", "--ckpt_path", type=str, metavar="", help="ckpt file", required=True)
    parser.add_argument("-mesh", "--mesh_file", type=str, metavar="", help="mesh file", required=True)
    parser.add_argument("-out", "--onnx_file", type=str, metavar="", help="onnx file", required=True)
